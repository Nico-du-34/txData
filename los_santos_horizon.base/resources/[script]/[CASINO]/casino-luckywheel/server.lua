...
RegisterNetEvent('luckywheel:getwheel', function()
	...
	if Config.LimitedSpins == true then 
		...
		if result == '0' then
			...
			TriggerClientEvent('QBCore:Notify', src, "Vous avez déjà tourné la roue aujourd'hui", "error")
		end
	elseif Config.LimitedSpins == false then
		if Player.PlayerData.money["bank"] >= Config.startingPrice then
			...
			TriggerClientEvent('QBCore:Notify', src, "Vous n'avez pas assez d'argent pour tourner la roue!", "error")
		end
	end
end)
...
RegisterNetEvent('luckywheel:give', function(source, price)
	...
	elseif price.type == 'item' then
		...
		TriggerClientEvent('QBCore:Notify', source, "Félicitations!Tu as gagné "..price.count.." "..price.name.."!", 'success')
	...
	elseif price.type == 'money' then
		...
		TriggerClientEvent('QBCore:Notify', source, "Félicitations!Tu as gagné "..price.count.." $", 'success')
	elseif price.type == 'weapon' then
		TriggerClientEvent("chCasinoWall:bigWin", source)
		Player.Functions.AddItem(price.name, 1, slot)
		TriggerClientEvent('QBCore:Notify', source, "Félicitations!Tu as gagné une arme!", 'success')
