...
function OpenApartmentMenu(_type)
    ...
    if not apartmentDatas or not next(apartmentDatas) then return SendTextMessage(Lang('HOUSING_NOTIFICATION_NO_APARTMENTS'), 'error') end
    local data = {}
    for k, v in pairs(apartmentDatas) do
        ...
        table.insert(data, {
            ...
            onSelect = function(args)
                ...
                SendTextMessage(Lang('HOUSING_NOTIFICATION_YOU_RING_DOOR'), 'inform')
...
function OpenMyApartments()
    ...
    if #data == 0 then return SendTextMessage(Lang('HOUSING_NOTIFICATION_NO_OWNED_APARTMENTS'), 'error') end
    lib.registerContext({
