...
local function startLooting(prop)
    ...
    if math.random(100) <= Config.moneyChance then
        ...
        AddNotification("💵 Tu as trouvé $" .. moneyAmount .. "! Trouvaille chanceuse!", 'success')
        ...
        if #availableLoot > 0 then
            ...
            AddNotification("🎉 Tu as trouvé: " .. loot.item .. " x" .. quantity .. "! Belle trouvaille!", 'success')
        else
            AddNotification("⚠️ Rien d'utile trouvé cette fois.", 'error')
        end
    end
    playLootSound()
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if nearestProp and propData then
            ...
            DrawText3D(propCoords.x, propCoords.y, propCoords.z, "💰 [E] Fouille!")
...
AddEventHandler("client:propAlreadyLooted", function()
    AddNotification("⚠️ Cet accessoire a déjà été pillé!", 'error')
end)
...
AddEventHandler("client:allowLooting", function()
    if lootingProp and lootingPropData then
        ...
        AddNotification("⚠️ Impossible de trouver l'accessoire à piller.", 'error')
    end
end)
