...
var quotes = [
  'Le projet Sloth en haut !',
  'Une réécriture de Discord résout tout...',
  'Quelqu\'un lit vraiment ces messages ?',
  'La meilleure façon de prédire votre avenir est de le créer.',
  'Croyez que vous pouvez et vous êtes déjà à moitié sur la route.',
  'En trois mots, je peux résumer tout ce que j\'ai appris de la vie : elle continue.',
  ...
  'Si vous voulez vivre une vie heureuse, attachez-la à un objectif, pas aux gens ni aux choses.',
  ...
  'La seule chose nécessaire à la victoire du mal est que les hommes de bien ne fassent rien.',
  'Peu importe la lenteur de votre progression, tant que vous ne vous arrêtez pas.',
  'Le meilleur moment pour planter un arbre était il y a 20 ans. Le deuxième meilleur moment, c\'est maintenant.',
  'Croyez en vous et en tout ce que vous êtes. Sachez qu\'il y a quelque chose en vous de plus grand que n\'importe quel obstacle.',
  ...
  'Ce ne sont pas les années dans votre vie qui comptent. Ce qui compte, c\'est la vie dans vos années.',
  'La plus grande gloire dans la vie ne réside pas dans le fait de ne jamais tomber, mais dans le fait de se relever à chaque fois que nous tombons.',
  'Les deux jours les plus importants de votre vie sont le jour où vous êtes né et le jour où vous découvrez pourquoi.',
  'Le succès ne dépend pas de la hauteur que vous avez atteinte, mais de la manière dont vous apportez une différence positive au monde.',
...
function timeAgo(dateParam) {
  ...
  if (seconds < 5) {
    return "À l'instant";
        ...
        return `${seconds} secondes ago`;  // "secondes" est déjà en français
    } else if (seconds < 90) {
        return "Il y a environ une minute";
    } else if (minutes < 60) {
        return `${minutes} minutes ago`;  // "minutes" est déjà en français
    } else if (isToday) {
    ...
    if (sentJob) {
      if (PoliceJobs[sentJob] !== undefined)  {
        if (sentJob == "police") {
          ...
          } else if (sentJob == "sherif"){
            applyCustomTheme(customThemes.sherif)
     ...
     /*  let fireCount = 0; */
      ...
      activeUnits.forEach((unit) => {
        ...
        if (PoliceJobs[unit.unitType] !== undefined) {
          ...
          } else if(unit.unitType == "sherif")  { sherifCount++;
            activeInfoJob = `<div class="unit-job active-info-job-sherif">sherif</div>`;
          } else if(unit.unitType == "lssd")  { sherifCount++;
            activeInfoJob = `<div class="unit-job active-info-job-sherif">LSSD</div>`;
